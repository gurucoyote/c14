...
bool LLCacheName::importFile(std::istream& istr)
{
	...
	for( ; iter != end; ++iter)
	{
		...
		fullname = cleanFullName(fullname); // Do not keep "Resident" for reverse lookups
...
{
	ReverseCache::iterator iter = impl.mReverseCache.find(cleanFullName(full_name));
...
void LLCacheName::Impl::processUUIDReply(LLMessageSystem* msg, bool isGroup)
{
	...
	for(S32 i = 0; i < count; ++i)
	{
		...
		if (!isGroup)
		{
			...
			full_name = cleanFullName(full_name); // Do not keep "Resident" for reverse lookups
