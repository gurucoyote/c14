...
void ll_debug_socket(const char* msg, apr_socket_t* apr_sock)
{
	...
	if (!apr_sock)
	{
		LL_DEBUGS("IOSocket") << "Socket -- " << (msg ? msg : "")
							  << ": no socket." << LL_ENDL;
		return;
	}
...
LLSocket::ptr_t LLSocket::create(EType type, U16 port)
{
	...
	if (STREAM_TCP == type)
	{
		status = apr_socket_create(&rv->mSocket, APR_INET, SOCK_STREAM,
								   APR_PROTO_TCP, rv->mPool());
	}
	else if (DATAGRAM_UDP == type)
	{
		status = apr_socket_create(&rv->mSocket, APR_INET, SOCK_DGRAM,
								   APR_PROTO_UDP, rv->mPool());
	}
	else
	{
		rv.reset();
	...
	if (port > 0)
	{
		...
		LL_DEBUGS("IOSocket") << "Bound " << (DATAGRAM_UDP == type ? "udp" : "tcp")
							  << " socket to port: " << sa->port << LL_ENDL;
		if (STREAM_TCP == type)
		{
			// If it's a stream based socket, we need to tell the OS
			// to keep a queue of incoming connections for ACCEPT.
			LL_DEBUGS("IOSocket") << "Setting listen state for socket." << LL_ENDL;
			status = apr_socket_listen(rv->mSocket,
...
LLSocket::ptr_t LLSocket::create(apr_status_t& status, LLSocket::ptr_t& listen_socket)
{
	...
	LLSocket::ptr_t rv(new LLSocket);
	LL_DEBUGS("IOSocket") << "accepting socket" << LL_ENDL;
...
{
	LLMemType m1(LLMemType::MTYPE_IO_TCP);
	//LL_DEBUGS("IOSocket") << "Destroying LLIOSocketReader" << LL_ENDL;
}
...
{
	LLMemType m1(LLMemType::MTYPE_IO_TCP);
	//LL_DEBUGS("IOSocket") << "Destroying LLIOSocketWriter" << LL_ENDL;
}
...
{
	LLMemType m1(LLMemType::MTYPE_IO_TCP);
	//LL_DEBUGS("IOSocket") << "Destroying LLIOServerSocket" << LL_ENDL;
}
	...
	LLPumpIO* pump)
{
	...
	if (!mInitialized)
	{
		...
		// from a remote host.
		LL_DEBUGS("IOSocket") << "Initializing poll descriptor for LLIOServerSocket."
							  << LL_ENDL;
		apr_pollfd_t poll_fd;
		poll_fd.p = NULL;
		poll_fd.desc_type = APR_POLL_SOCKET;
	...
	{
		char buf[256];
		llwarns << "Unable to accept linden socket: "
				<< apr_strerror(status, buf, sizeof(buf)) << llendl;
	}
...
#if 0
LLIODataSocket::LLIODataSocket(U16 suggested_port, U16 start_discovery_port)
:	mSocket(NULL)
{
	...
	if (sa->port)
	{
		LL_DEBUGS("IOSocket") << "Bound datagram socket to port: "
							  << sa->port << LL_ENDL;
		mPort = sa->port;
	}
	else
