...
{
	if (!mImage || mImage->getFullWidth() * mImage->getFullHeight() == 0) return;
	...
	while (client_width > max_client_width || client_height > max_client_height)
	{
		client_width /= 2;
		client_height /= 2;
	}
...
void LLPreviewTexture::onRefreshBtn(void* data)
{
	...
	if (self->mImage)
	{
		...
		LLAppViewer::getTextureCache()->removeFromCache(self->mImage->getID());
