...
static void settings_to_globals()
{
	...
	LLVOAvatar::sAvatarPhysics			= gSavedSettings.getBOOL("AvatarPhysics");
	...
	while (!LLApp::isExiting())
	{
		...
		try
		{
			...
			if (gViewerWindow)
			{
				...
				if (!restoreErrorTrap())
				{
					LL_WARNS("AppInit") << " Someone took over my signal/exception handler (post messagehandling)!" << LL_ENDL;
				}
...
//MK
					...
					&& gViewerWindow && !gViewerWindow->getShowProgress())
				{
					...
					if (!gAgent.mRRInterface.mAssetsToReattach.empty())
					{
						...
						static LLCachedControl<U32> reattach_delay(gSavedSettings, "RestrainedLoveReattachDelay");
						// Timeout flag.
						BOOL timeout = (gAgent.mRRInterface.mReattaching && elapsed > 4 * reattach_delay);
						if (timeout)
						{
							...
							LL_WARNS("RestrainedLove") << "Timeout reattaching an asset, retrying." << LL_ENDL;
						}
						if (!gAgent.mRRInterface.mReattaching || timeout)
						{
							...
							if (elapsed >= reattach_delay)
							{
								...
								LL_INFOS("RestrainedLove") << "Reattaching asset " << tmp_uuid
														   << " to point " << tmp_attachpt_nb << LL_ENDL;
...
//mk
			...
			// Sleep and run background threads
			{
				...
				{
					LL_INFOS("AppInit") << "Periodic slow frame - sleeping 500 ms" << LL_ENDL;
					ms_sleep(500);
				}
...
bool LLAppViewer::initConfiguration()
{
	...
	LL_INFOS("AppInit") << "Loading settings file list" << settings_file_list << LL_ENDL;
	if (0 == settings_control.loadFromFile(settings_file_list))
	{
        LL_ERRS("AppInit") << "Cannot load default configuration file " << settings_file_list << LL_ENDL;
	}
	...
	if (!initParseCommandLine(clp))
	{
		...
		LL_INFOS("AppInit")	<< "Command	line usage:\n" << clp << LL_ENDL;
	...
	if (clp.hasOption("settings"))
	{
		...
		LL_INFOS("AppInit")	<< "Using command line specified settings filename: " 
	...
	{
		LL_WARNS("AppInit") << "Crashes will be handled by system, stack trace logs and crash logger are both disabled"
							<< LL_ENDL;
	...
	if (clp.hasOption("help"))
	{
		...
		LL_INFOS("AppInit")	<< msg.str() << LL_ENDL;

		OSMessageBox(msg.str().c_str(), LLStringUtil::null, OSMB_OK);
    ...
    if (clp.hasOption("setdefault"))
    {
        ...
        //{
        //    LL_WARNS("AppInit") << "Invalid '--setdefault' parameter count." << LL_ENDL;
        //}
        //else
        //{
        ...
        //        else
        //        {
        //            LL_WARNS("AppInit") << "'--setdefault' specified with unknown setting: '"
        //                << name << "'." << LL_ENDL;
        //        }
        //    }
        //}
    ...
    if (clp.hasOption("set"))
    {
        ...
        {
            LL_WARNS("AppInit") << "Invalid '--set' parameter count." << LL_ENDL;
        }
        else
        {
            LLCommandLineParser::token_vector_t::const_iterator itr;
            for (itr = set_values.begin(); itr != set_values.end(); ++itr)
            {
                const std::string& name = *itr;
                const std::string& value = *(++itr);
                LLControlVariable* c = gSettings[sGlobalSettingsName]->getControl(name);
                ...
                {
                    LL_WARNS("AppInit") << "'--set' specified with unknown setting: '"
										<< name << "'." << LL_ENDL;
                }
            }
        }
	...
	//	{
	//		LL_ERRS("AppInit") << "Failed to parse skin definition." << LL_ENDL;
	//	}
...
#endif
    {
        LL_INFOS("AppInit") << "Last execution froze, requesting to send crash report." << LL_ENDL;
        //
        ...
        if (OSBTN_YES == choice)
        {
            LL_INFOS("AppInit") << "Sending crash report." << LL_ENDL;
        ...
        {
            LL_INFOS("AppInit") << "Not sending crash report." << LL_ENDL;
        }
    }
...
void LLAppViewer::writeDebugInfo()
{
	...
	LL_INFOS("AppInit") << "Opening debug file " << debug_filename << LL_ENDL;
...
{
	LL_INFOS("AppInit") << "Handle viewer crash entry." << LL_ENDL;
...
// 	pApp->destroyMainloopTimeout(); // SJB: Bah. This causes the crash handler to hang, not sure why.
	...
	if (gDirUtilp)
	{
		...
		{
			LL_WARNS("AppInit") << "Cannot create error marker file " << crash_file_name << LL_ENDL;
		}		
	}
	...
	if (gMessageSystem && gDirUtilp)
	{
		...
		if (file.good())
		{
			LL_INFOS("AppInit") << "Handle viewer crash generating stats log." << LL_ENDL;
...
{
	LL_INFOS("AppInit") << "requestQuit" << LL_ENDL;
...
{
    LL_INFOS("AppInit") << "abortQuit()" << LL_ENDL;
	mQuitRequested = false;
}
...
void LLAppViewer::migrateCacheDirectory()
{
	...
	if (gSavedSettings.getBOOL("MigrateCacheDirectory"))
	{
		...
		{
			LL_INFOS("AppInit") << "Migrating cache from " << old_cache_dir << " to " << new_cache_dir << LL_ENDL;
			...
			LL_INFOS("AppInit") << "Moved " << file_count << " files" << LL_ENDL;
			...
			{
				LL_WARNS("AppInit") << "could not delete old cache directory " << old_cache_dir << LL_ENDL;
			}
		}
	}
...
bool LLAppViewer::initCache()
{
	...
	if (stat_result)
	{
		...
		if (gDirUtilp->getNextFileInDir(dir, mask, found_file, false))
		{
			...
			LL_DEBUGS("AppCache") << "Default vfs data file not present, found: "
								  << old_vfs_data_file << " Old salt: " << old_salt
								  << LL_ENDL;
		}
	}
...
void LLAppViewer::idle()
{
...

	{
		...
		{
			LL_INFOS("AppStats") << "Transmitting sessions stats" << LL_ENDL;
			send_stats();
		...
		if (object_debug_timer.getElapsedTimeF32() > 5.f)
		{
			...
			{
				LL_INFOS("AppStats") << "Dead object updates: " << gObjectList.mNumDeadObjectUpdates << LL_ENDL;
				gObjectList.mNumDeadObjectUpdates = 0;
			}
			if (gObjectList.mNumUnknownKills)
			{
				LL_INFOS("AppStats") << "Kills on unknown objects: " << gObjectList.mNumUnknownKills << LL_ENDL;
				gObjectList.mNumUnknownKills = 0;
			}
			if (gObjectList.mNumUnknownUpdates)
			{
				LL_INFOS("AppStats") << "Unknown object updates: " << gObjectList.mNumUnknownUpdates << LL_ENDL;
...
public:
	...
	{		
		// consider retries
		LL_INFOS("AppInit") << "LLLogoutResponder error "
	...
	void result(const LLSD& content)
	{
		...
		LL_INFOS("AppInit") << "LLLogoutResponder completed successfully" << LL_ENDL;
	}
...
void LLAppViewer::idleNetwork()
{
	...
	if (!speed_test)
	{
		...
		{
			LL_INFOS("MessagesPerFrame") << "Maxed out number of messages per frame at " << MESSAGE_MAX_PER_FRAME << LL_ENDL;
		}

		if (gPrintMessagesThisFrame)
		{
			LL_INFOS("MessagesPerFrame") << "Decoded " << total_decoded << " msgs this frame!" << LL_ENDL;
...
void LLAppViewer::disconnectViewer()
{
	...
	LL_INFOS("AppInit") << "Disconnecting viewer!" << LL_ENDL;
