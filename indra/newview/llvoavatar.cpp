...
{
	LLInventoryItem* item = gInventory.getItem(gInventory.getLinkedItemID(item_id));
	if (item)
	{
...
void LLVOAvatar::idleUpdateMisc(bool detailed_update)
{
	...
	if (isImpostor() && !mNeedsImpostorUpdate)
	{
		...
		{	//update impostor if view angle, distance, or bounding box change
			...
			if (mImpostorDistance != 0.0f && dist_diff / mImpostorDistance > 0.1f)
			{
				mNeedsImpostorUpdate = TRUE;
			}
			else
...
void LLVOAvatar::idleUpdateNameTag(const LLVector3& root_pos_last)
{
	...
	static LLCachedControl<bool> show_typing_info(gSavedSettings, "ShowTypingInfo");
	...
	if (mIsSelf)
	{
		...
		render_name = render_name && !gAgent.cameraMouselook() &&
...
//mk
	if (render_name)
	{
		...
		if (mNameText.notNull() && firstname && lastname)
		{
			...
			{
				static LLCachedControl<bool> small_avatar_names(gSavedSettings, "SmallAvatarNames");
...
//							AUDIO_STEP_LO_GAIN, AUDIO_STEP_HI_GAIN);
			...
			if (LLViewerParcelMgr::getInstance()->canHearSound(foot_pos_global)
				&& (!ml || !ml->isMuted(getID(), LLMute::flagObjectSounds)))
			{
...
void LLVOAvatar::resetSpecificJointPosition(const std::string& name)
{
	...
	{
		llinfos << "Did not find joint: " << name.c_str() << llendl;
	}
}
...
std::string LLVOAvatar::getFullname() const
{
	...
	if (first && last)
	{
		...
		if (!last_name.empty() && !(LLAvatarName::sOmitResidentAsLastName && last_name == "Resident"))
		{
			name += " ";
			name += last_name;
		}
