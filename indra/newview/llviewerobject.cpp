...
LLBBox LLViewerObject::getBoundingBoxAgent() const
{
	...
	if (avatar_parent && avatar_parent->isAvatar() && root_edit &&
		root_edit->mDrawable.notNull() &&
...
void LLViewerObject::unpackParticleSource(const S32 block_num, const LLUUID& owner_id)
{
	...
	else
	{
		...
		if (ml && ml->isMuted(owner_id, LLMute::flagParticles)) return;
...
// If in_use is TRUE, will create a new extra parameter if none exists.
