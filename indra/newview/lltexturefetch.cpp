							  ...
							  const LLIOPipe::buffer_ptr_t& buffer)
	{
		...
		if (worker)
		{
			...
			if (!success)
			{
				worker->setGetStatus(status, reason);
 				LL_WARNS("Texture") << "CURL get failed for " << mID << " with status: " << status << LL_ENDL;
			}
		...
		else
		{
			mFetcher->removeFromHTTPQueue(mID);
 			LL_WARNS("Texture") << "Worker not found for " << mID << LL_ENDL;
		}
	}
...
{
// 	llassert_always(priority >= 0 && priority <= LLViewerFetchedTexture::maxDecodePriority());
...
// 		if (mHost != LLHost::invalid) use_http = false;
		...
		{
			mFetcher->removeFromNetworkQueue(this, false); // failsafe
			mState = SEND_HTTP_REQ;
			setPriority(LLWorkerThread::PRIORITY_HIGH | mWorkPriority);
...
// 				llwarns << "processSimulatorPackets() failed to load buffer" << llendl;
	...
	if (mState == SEND_HTTP_REQ)
	{
		if (mCanUseHTTP)
		{
			static LLCachedControl<S32> max_num_of_http_requests_in_queue(gSavedSettings, "TextureMaxHTTPRequests");
