...
void LLAgent::propagate(const F32 dt)
{
	...
	if (automatic_fly && mAvatarObject.notNull() && !mAvatarObject->mInAir && mUpKey < 0)
	{
...
void LLAgent::processAgentInitialWearablesUpdate(LLMessageSystem* mesgsys, void** user_data)
{
	...
	if (avatar && (agent_id == avatar->getID()))
	{
		...
		for (i = 0; i < num_wearables; i++)
		{
			...
			LL_DEBUGS("Wearables") << "       " << LLWearable::typeToTypeLabel(type)
								   << " " << asset_id << " item id "
								   << gAgent.mWearableEntry[type].mItemID.asString()
								   << LL_ENDL;
		}
		...
		for (i = 0; i < WT_COUNT; i++)
		{
			...
			{
				gWearableList.getAsset(asset_id_array[i],
									   LLStringUtil::null,
									   LLWearable::typeToAssetType((EWearableType)i),
									   LLAgent::onInitialWearableAssetArrived,
									   (void*)(intptr_t)i);
			}
		}
	}
...
void LLAgent::createStandardWearables(BOOL female)
{
	...
	for (S32 i=0; i < WT_COUNT; i++)
	{
		...
		if (create[i])
		{
			...
			LLPointer<LLInventoryCallback> cb =
				new addWearableToAgentInventoryCallback(donecb, i, wearable,
		...
		for (i = 0; i < count; ++i)
		{
			...
			if (old_wearable)
			{
				...
				if (no_link || isWearableCopyable((EWearableType)index))
				{
					...
					{
						move_inventory_item(gAgent.getID(),
											gAgent.getSessionID(),
											item->getLinkedUUID(),
											folder_id,
											new_name,
...
void LLAgent::sendAgentSetAppearance()
{
	...
	const bool physics_on = LLVOAvatar::sAvatarPhysics && (getWearable(WT_PHYSICS) != NULL);
	for (LLViewerVisualParam* param = (LLViewerVisualParam*)mAvatarObject->getFirstVisualParam();
		 param;	 param = (LLViewerVisualParam*)mAvatarObject->getNextVisualParam())
	{
		if (param->getGroup() == VISUAL_PARAM_GROUP_TWEAKABLE) // do not transmit params of group VISUAL_PARAM_GROUP_TWEAKABLE_NO_TRANSMIT
		{
			// A hack to prevent ruthing on older viewers when physics is not in use for our avatar.
								...
								BOOL remove)
{
	...
	wearables_to_remove[WT_PHYSICS]		= remove;
