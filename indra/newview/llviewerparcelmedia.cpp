...
void LLViewerParcelMedia::filterMedia(LLParcel* parcel, U32 type)
{
	...
	else
	{
		...
		LLNotifications::instance().add("MediaAlert", args, LLSD(),
										boost::bind(callback_media_alert,
													_1, _2, parcel, type,
													domain));
	}
}
...
void callback_media_alert(const LLSD &notification, const LLSD &response,
						  LLParcel* parcel, U32 type, std::string domain)
{
	S32 option = LLNotification::getSelectedOption(notification, response);

	std::string ip = LLViewerParcelMedia::getDomainIP(domain);
...
{
	std::string medialist_filename = gDirUtilp->getExpandedFilename(LL_PATH_PER_SL_ACCOUNT,
																	"media_filter.xml");
