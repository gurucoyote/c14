	...
	mGroupID(group_id)
{
	...
	mPanelGroupp = new LLPanelGroup("panel_group.xml", "PanelGroup", group_id, tab_name);
...
{
	LLFloaterGroupInfo *fgi = get_if_there(sInstances, group_id,
										   (LLFloaterGroupInfo*)NULL);
	if (fgi)
	{
		if (fgi->mPanelGroupp)
		{
...
// static
void LLFloaterGroupInfo::callbackLoadGroupName(const LLUUID& id, const std::string& name, bool is_group)
{
	LLFloaterGroupInfo *fgi = get_if_there(sInstances, id,
										   (LLFloaterGroupInfo*)NULL);
	if (fgi)
	{
