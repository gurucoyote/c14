...
void LLConsole::reshape(S32 width, S32 height, BOOL called_from_parent)
{
	...
	if (mConsoleWidth == new_width && mConsoleHeight == new_height)
	{
		return;
	}
	...
	for (paragraph_t::iterator paragraph_it = mParagraphs.begin();
		 paragraph_it != mParagraphs.end(); paragraph_it++)
	{
...
void LLConsole::draw()
{
	...
	static LLCachedControl<bool> console_box_per_message(gSavedSettings, "ConsoleBoxPerMessage");
	if (!console_box_per_message)
	{
		// This section makes a single huge black box behind all the text.
		S32 bkg_height = 4;
		static LLCachedControl<bool> disable_messages_spacing(gSavedSettings, "DisableMessagesSpacing");
		if (disable_messages_spacing)
		{
			message_spacing = 0;
			bkg_height = 8;
		}
...
void LLConsole::Paragraph::updateLines(F32 screen_width, LLFontGL* font, bool force_resize)
{
	if (!force_resize)
	{
		if (mMaxWidth >= 0.0f && mMaxWidth < screen_width)
		{
	...
	if (mParagraphText.empty() || mParagraphColorSegments.empty() || font == NULL)
	{
