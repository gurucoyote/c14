...
#include "llpanelgroupvoting.h"		// #ifdef GROUP_VOTING
...
void open_offer(const std::vector<LLUUID>& items, const std::string& from_name)
{
	...
	for ( ; it != end; ++it)
	{
		...
		if (gInventory.isObjectDescendentOf(item->getUUID(), lost_and_found_id) && !user_is_away)
		{
			return;
		}
...
bool LLOfferInfo::inventory_offer_callback(const LLSD& notification, const LLSD& response)
 {
	...
	if (button == IOR_MUTE)
	{
		gCacheName->get(mFromID, mFromGroup,
						boost::bind(&inventory_offer_mute_callback, _1, _2, _3));
	}
...
//MK
	...
	if (gRRenabled)
	{
		...
		if (gAgent.mRRInterface.mContainsShowloc)
		{
			...
			mDesc = gAgent.mRRInterface.stringReplace (mDesc, gAgent.getRegion()->getName(), "(Region hidden)");
		}
	}
//mk
...
void inventory_offer_handler(LLOfferInfo* info, BOOL from_task)
{
	...
	if (from_task)
	{
		...
		if (info->mFromGroup)
		{
			if (gCacheName->getGroupName(info->mFromID, name))
			{
				name_found = true;
			}
		}
		...
		{
			if (gCacheName->getFullName(info->mFromID, name))
			{
				name_found = true;
			}
		}
		...
		muted = ml && ml->isMuted(LLUUID::null, info->mFromName, 0, LLMute::OBJECT);
		if (!muted && ml)
		{
			if (name_found)
			{
				// Search for mutes by owner's group or agent UUID and name
				muted = ml->isMuted(info->mFromID, name);
			}
			else
			{
				// Search for mutes by owner's group or agent UUID
				muted = ml->isMuted(info->mFromID);
			}
		}
	}
	else
	{
		name = LLAvatarName::sOmitResidentAsLastName ?
				LLCacheName::cleanFullName(info->mFromName) : info->mFromName;

		// Search for mutes by group or agent id or name
		muted = ml && ml->isMuted(info->mFromID, name);
	}

	// If muted, don't even go through the messaging stuff.
...
void process_improved_im(LLMessageSystem *msg, void **user_data)
{
	...
	BOOL is_away = gAgent.getAFK() && gSavedPerAccountSettings.getBOOL("BusyResponseWhenAway");
...
//mk
	...
	case IM_NOTHING_SPECIAL: 
		...
		if (!gAgent.isGodlike() && gAgent.getRegion()->isPrelude() && to_id.isNull())
		{
			// do nothing -- don't distract newbies in
			// Prelude with global IMs
		}
		else if ((is_busy || is_away) && offline == IM_ONLINE && !is_linden && name != SYSTEM_FROM
//MK
...
//mk
		{
			...
			{
				// if there is not a panel for this conversation (i.e. it is a
				// new IM conversation initiated by the other party) then...
				std::string my_name;
				...
				pack_instant_message(gMessageSystem, gAgent.getID(), FALSE,
									 gAgent.getSessionID(), from_id, my_name,
									 response, IM_ONLINE, IM_BUSY_AUTO_RESPONSE,
									 session_id);
...
//MK
		else if (gRRenabled && (message == "@version" || message == "@getblacklist"))
		{
			...
			if (message == "@version")
			{
				response = gAgent.mRRInterface.getVersion();
			}
			else
			{
				response = RRInterface::sBlackList;
			}
...
//MK
					...
					|| gAgent.mRRInterface.contains("recvimfrom:" + from_id.asString())))
			{
				...
				std::string my_name;
				gAgent.buildFullname(my_name);
				my_name += " using viewer " + gAgent.mRRInterface.getVersion();
	...
	case IM_FROM_TASK:
		{
			if ((is_busy && !is_owned_by_me) ||
				...
				ml->isMuted(session_id, name, LLMute::flagTextChat))
			{
				return;
			}
			chat.mText = name + separator_string + message.substr(message_offset);
...
//mk
			...
			// NOT an IM, because IMs from objects don't open IM sessions.
...
void busy_message(LLMessageSystem* msg, LLUUID from_id) 
{
	if (gAgent.getBusy())
	{
		...
		response += gSavedPerAccountSettings.getText("BusyModeResponse");
		pack_instant_message(gMessageSystem, gAgent.getID(), FALSE,
							 gAgent.getSessionID(), from_id, my_name, response,
							 IM_ONLINE, IM_BUSY_AUTO_RESPONSE);
		gAgent.sendReliableMessage();
	}
}
...
void process_offer_callingcard(LLMessageSystem* msg, void**)
{
	...
	if (source && source->isAvatar())
	{
		...
		if (nvfirst && nvlast)
		{
			source_name = LLCacheName::buildFullName(nvfirst->getString(), nvlast->getString());
	...
	if (!source_name.empty())
	{
		LLMuteList* ml = LLMuteList::getInstance();
		if (gAgent.getBusy() ||
			(ml && ml->isMuted(source_id, source_name, LLMute::flagTextChat)))
		{
...
//mk
	...
	LLMuteList* ml = LLMuteList::getInstance();
	BOOL is_muted = ml && (ml->isMuted(from_id, from_name, LLMute::flagTextChat)
					|| ml->isMuted(owner_id, LLMute::flagTextChat));

	BOOL is_linden = chat.mSourceType != CHAT_SOURCE_OBJECT &&
					 ml && ml->isLinden(from_name);
...
void process_sound_trigger(LLMessageSystem *msg, void **)
{
	...
	LLMuteList* ml = LLMuteList::getInstance();
	if (ml &&
		(ml->isMuted(owner_id, LLMute::flagObjectSounds) ||	// sound from some muted resident
		 ml->isMuted(object_id) ||							// sound from a muted object
		 (parent_id.notNull() && ml->isMuted(parent_id)) ||	// sound from object whose parent is muted
		 !gAgent.canAccessMaturityInRegion(region_handle)))	// sound from region with maturity above current agent maturity
	{
		return;
	}
...
void process_preload_sound(LLMessageSystem *msg, void **user_data)
{
	...
	LLMuteList* ml = LLMuteList::getInstance();
	if (ml && (ml->isMuted(object_id) ||
		ml->isMuted(owner_id, LLMute::flagObjectSounds)))
	{
		return;
	}
...
void process_script_question(LLMessageSystem *msg, void **user_data)
{
	...
	// by id, by name, or by owner name (agent or group)
	LLMuteList* ml = LLMuteList::getInstance();
	if (ml && (ml->isMuted(taskid, object_name) ||
		ml->isMuted(LLUUID::null, owner_name, 0, LLMute::AGENT) ||
		ml->isMuted(LLUUID::null, owner_name, 0, LLMute::GROUP)))
	{
		return;
	}
...
//MK
		if (gRRenabled)
		{
			if (gAgent.mRRInterface.containsWithoutException("sendim"))
			{
				text = "(Hidden)";
			}
			else
			{
				...
				{
					if (gAgent.mRRInterface.containsSubstr("sendimto:" + it->asUUID().asString()))
					{
						text = "(Hidden)";
						break;
					}
