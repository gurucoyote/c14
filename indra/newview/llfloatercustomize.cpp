...
BOOL LLPanelEditWearable::postBuild()
{
	...
	mCanTakeOff = LLWearable::typeToAssetType(mType) == LLAssetType::AT_CLOTHING &&
				  !(gAgent.isTeen() && (mType == WT_UNDERSHIRT || mType == WT_UNDERPANTS));
	if (getChild<LLButton>("Take Off", TRUE, FALSE))
	{
...
void LLPanelEditWearable::setSubpart(ESubpart subpart)
{
	...
	{
		LLButton* btn = getChild<LLButton>(iter->second->mButtonName, TRUE, FALSE);
		if (btn)
		{
...
void LLPanelEditWearable::draw()
{
		 ...
		 iter != mSubpartList.end(); ++iter)
	{
		...
		{
			childSetEnabled(btn_name, iter->second->mSex & avatar->getSex());
		}
		else
		{
...
LLScrollingPanelParam::LLScrollingPanelParam(const std::string& name, LLViewerJointMesh* mesh,
											 LLViewerVisualParam* param, BOOL allow_modify,
											 bool bVisualHint)
	...
	mHintMax(NULL)
{
	...
	if	(bVisualHint)
	{
		...
		if (getChild<LLUICtrl>("min param text", TRUE, FALSE))
		{
			// *TODO::translate
...
void LLScrollingPanelParam::updatePanel(BOOL allow_modify)
{
	...
	if (mHintMin)	mHintMin->requestUpdate(sUpdateDelayFrames++);
	if (mHintMax)	mHintMax->requestUpdate(sUpdateDelayFrames++);

	mAllowModify = allow_modify;
	childSetEnabled("param slider", mAllowModify);
	if (mLess)	childSetEnabled("less", mAllowModify);
	if (mMore)	childSetEnabled("more", mAllowModify);
}
...
void LLScrollingPanelParam::setVisible(BOOL visible)
{
	...
	{
		LLPanel::setVisible(visible);
		if (mHintMin)	mHintMin->setAllowsUpdates(visible);
		if (mHintMax)	mHintMax->setAllowsUpdates(visible);

		if (visible)
		{
			if (mHintMin)	mHintMin->setUpdateDelayFrames(sUpdateDelayFrames++);
			if (mHintMax)	mHintMax->setUpdateDelayFrames(sUpdateDelayFrames++);
		}
	}
}
...
void LLScrollingPanelParam::onHintMinMouseUp(void* userdata)
{
	...
	if (avatar)
	{
		...
		if (hint && elapsed_time < PARAM_STEP_TIME_THRESHOLD)
		{
			...
			LLSliderCtrl* slider = self->getChild<LLSliderCtrl>("param slider", TRUE, FALSE);
			if (slider)
			{
				if (slider->getMinValue() < new_percent
					&& new_percent < slider->getMaxValue())
...
BOOL LLFloaterCustomize::postBuild()
{
	...
	if (gAgent.isTeen())
	{
		LLTabContainer* tab_container = getChild<LLTabContainer>("customize tab container", TRUE, FALSE);
		if (tab_container)
		{
			LLPanel* panel;
												  ...
												  bool bVisualHint)
{
	...
	if (mScrollingPanelList)
	{
		...
		{
			mScrollingPanelList->addPanel(new LLScrollingPanelParam("LLScrollingPanelParam",
																	joint_mesh, (*it).second.second,
																	(*it).second.first, bVisualHint));
		}
	}
}
