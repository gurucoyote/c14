...
/*
void unpack_request_params(LLMessageSystem* msg,
...
void LLFloaterRegionInfo::refreshFromRegion(LLViewerRegion* region)
{
	...
	std::for_each(mInfoPanels.begin(), mInfoPanels.end(), llbind2nd(
#if LL_WINDOWS
...
{
	if (getChild<LLButton>("apply_btn", TRUE, FALSE))
	{
...
{
	if (getChild<LLButton>(btn_name, TRUE, FALSE))
	{
...
void LLPanelRegionGeneralInfo::onClickKickAll(void* userdata)
{
	...
	LLNotifications::instance().add("KickUsersFromRegion", LLSD(), LLSD(), 
...
void LLPanelRegionGeneralInfo::onClickMessage(void* userdata)
{
	...
	LLNotifications::instance().add("MessageRegion", LLSD(), LLSD(), 
...
bool LLPanelRegionTextureInfo::refreshFromRegion(LLViewerRegion* region)
{
	...
	for (S32 i = 0; i < TERRAIN_TEXTURE_COUNT; ++i)
	{
		...
		texture_ctrl = getChild<LLTextureCtrl>(buffer, TRUE, FALSE);
		if (texture_ctrl)
		{
...
BOOL LLPanelRegionTextureInfo::sendUpdate()
{
	...
	for (S32 i = 0; i < CORNER_COUNT; ++i)
	{
		...
		std::string buffer3 = llformat("%d %f %f", i, (F32)childGetValue(buffer).asReal(),
									   (F32)childGetValue(buffer2).asReal());
...
BOOL LLPanelRegionTextureInfo::validateTextureSizes()
{
	for (S32 i = 0; i < TERRAIN_TEXTURE_COUNT; ++i)
	{
		std::string buffer;
		buffer = llformat("texture_detail_%d", i);
		LLTextureCtrl* texture_ctrl = getChild<LLTextureCtrl>(buffer, TRUE, FALSE);
		...
		//llinfos << "texture detail " << i << " is "
		//		 << width << "x" << height << "x" << components << llendl;
...
BOOL LLPanelRegionTerrainInfo::sendUpdate()
{
	...
	LLTabContainer* tab = floater->getChild<LLTabContainer>("region_panels", TRUE, FALSE);
	if (!tab) return true;

	LLPanelEstateInfo* panel = (LLPanelEstateInfo*)tab->getChild<LLPanel>("Estate", TRUE, FALSE);
...
void LLPanelRegionTerrainInfo::onClickBakeTerrain(void* data)
{
	LLNotifications::instance().add(
		...
		.functor(boost::bind(&LLPanelRegionTerrainInfo::callbackBakeTerrain,
...
void LLPanelEstateInfo::onClickRemoveAllowedAgent(void* user_data)
{
	accessRemoveCore(ESTATE_ACCESS_ALLOWED_AGENT_REMOVE,
					 "EstateAllowedAgentRemove", "allowed_avatar_name_list");
}
...
void LLPanelEstateInfo::onClickRemoveAllowedGroup(void* user_data)
{
	accessRemoveCore(ESTATE_ACCESS_ALLOWED_GROUP_REMOVE,
					 "EstateAllowedGroupRemove", "allowed_group_name_list");
}
...
void LLPanelEstateInfo::onClickRemoveBannedAgent(void* user_data)
{
	accessRemoveCore(ESTATE_ACCESS_BANNED_AGENT_REMOVE,
					 "EstateBannedAgentRemove", "banned_avatar_name_list");
}
...
void LLPanelEstateInfo::onClickRemoveEstateManager(void* user_data)
{
	accessRemoveCore(ESTATE_ACCESS_MANAGER_REMOVE,
					 "EstateManagerRemove", "estate_manager_name_list");
}
...
void LLPanelEstateInfo::onClickKickUser(void *user_data)
{
	...
	// in order to set up floater dependency
	LLFloater* parent_floater = gFloaterView->getParentFloater(panelp);
	LLFloater* child_floater = LLFloaterAvatarPicker::show(LLPanelEstateInfo::onKickUserCommit,
...
void LLPanelEstateInfo::onKickUserCommit(const std::vector<std::string>& names, const std::vector<LLUUID>& ids, void* userdata)
{
	...
	if (ids[0].isNull() || names[0].length() == 0)
	{
		return;
	}
	...
	LLNotifications::instance().add("EstateKickUser", args, payload,
									boost::bind(&LLPanelEstateInfo::kickUserConfirm,
												self, _1, _2));
...
bool LLPanelEstateInfo::kickUserConfirm(const LLSD& notification, const LLSD& response)
{
	...
	if (option == 0)
	{
		...
		sendEstateOwnerMessage(gMessageSystem, "kickestate",
...
void LLPanelEstateInfo::accessRemoveCore(U32 operation_flag, const std::string& dialog_name, const std::string& list_ctrl_name)
{
	LLPanelEstateInfo* panel = LLFloaterRegionInfo::getPanelEstate();
	if (!panel) return;
	LLNameListCtrl* name_list = panel->getChild<LLNameListCtrl>(list_ctrl_name, TRUE, FALSE);
	if (!name_list) return;
...
BOOL LLPanelEstateInfo::postBuild()
{
	...
	LLNameListCtrl *avatar_name_list = getChild<LLNameListCtrl>("allowed_avatar_name_list", TRUE, FALSE);
	if (avatar_name_list)
	{
	...
	LLNameListCtrl* group_name_list = getChild<LLNameListCtrl>("allowed_group_name_list", TRUE, FALSE);
	if (group_name_list)
	{
	...
	LLNameListCtrl* banned_name_list = getChild<LLNameListCtrl>("banned_avatar_name_list", TRUE, FALSE);
	if (banned_name_list)
	{
	...
	LLNameListCtrl* manager_name_list = getChild<LLNameListCtrl>("estate_manager_name_list", TRUE, FALSE);
...
{
public:
	LLEstateChangeInfoResponder(void* userdata)
		: mpPanel((LLPanelEstateInfo*)userdata) {};
...
void LLPanelEstateInfo::setEstateFlags(U32 flags)
{
	...
	childSetValue("voice_chat_check", LLSD(flags & REGION_FLAGS_ALLOW_VOICE ? TRUE : FALSE));
...
{
	LLNameListCtrl* name_list = getChild<LLNameListCtrl>("allowed_avatar_name_list", TRUE, FALSE);
	if (name_list)
	{
		name_list->deleteAllItems();
	}

	name_list = getChild<LLNameListCtrl>("banned_avatar_name_list", TRUE, FALSE);
	if (name_list)
	{
		name_list->deleteAllItems();
	}
...
BOOL LLPanelEstateInfo::checkRemovalButton(std::string name)
{
	...
	LLNameListCtrl* name_list = getChild<LLNameListCtrl>(name, TRUE, FALSE);
	if (name_list)
	{
		childSetEnabled(btn_name, name_list->getFirstSelected() ? TRUE : FALSE);
	}
...
{
	LLTextBox* region_name = getChild<LLTextBox>("region_name_text", TRUE, FALSE);
	if (region_name)
	{
		region_name->setText(region->getName());
	}

	LLTextBox* resellable_clause = getChild<LLTextBox>("resellable_clause", TRUE, FALSE);
	...
	LLTextBox* changeable_clause = getChild<LLTextBox>("changeable_clause", TRUE, FALSE);
	...
	LLTextBox* region_maturity = getChild<LLTextBox>("region_maturity_text", TRUE, FALSE);
	if (region_maturity)
	{
		region_maturity->setText(region->getSimAccessString());
	}

	LLTextBox* region_landtype = getChild<LLTextBox>("region_landtype_text", TRUE, FALSE);
	if (region_landtype)
	{
...
// strings[8] = bin(uuid)
// ...
bool LLDispatchSetEstateAccess::operator()(const LLDispatcher* dispatcher,
										   const std::string& key,
										   const LLUUID& invoice,
										   const sparam_t& strings)
{
	...
	if (access_flags & ESTATE_ACCESS_ALLOWED_AGENTS)
	{
		...
		allowed_agent_name_list = panel->getChild<LLNameListCtrl>("allowed_avatar_name_list", TRUE, FALSE);
	...
	if (access_flags & ESTATE_ACCESS_ALLOWED_GROUPS)
	{
		...
		allowed_group_name_list = panel->getChild<LLNameListCtrl>("allowed_group_name_list", TRUE, FALSE);
	...
	if (access_flags & ESTATE_ACCESS_BANNED_AGENTS)
	{
		...
		banned_agent_name_list = panel->getChild<LLNameListCtrl>("banned_avatar_name_list", TRUE, FALSE);
	...
	if (access_flags & ESTATE_ACCESS_MANAGERS)
	{
		...
		LLNameListCtrl* estate_manager_name_list =
			panel->getChild<LLNameListCtrl>("estate_manager_name_list", TRUE, FALSE);
		...
		{	
			...
			panel->childSetEnabled("remove_estate_manager_btn",
								   estate_manager_name_list->getFirstSelected() ? TRUE : FALSE);
