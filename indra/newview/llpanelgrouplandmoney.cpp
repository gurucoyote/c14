...
bool LLPanelGroupLandMoney::impl::applyContribution()
{
	...
	S32 new_contribution = mYourContributionEditorp ?
						   atoi(mYourContributionEditorp->getText().c_str()) : -1;
...
void LLPanelGroupLandMoney::activate()
{
	if (!mImplementationp->mBeenActivated)
	{
		...
		LLTabContainer* tabp = getChild<LLTabContainer>("group_money_tab_container", TRUE, FALSE);
		if (tabp)
		{
			tabp->selectFirstTab();
...
BOOL LLPanelGroupLandMoney::postBuild()
{
	...
	*/
	...
	mImplementationp->mMapButtonp = getChild<LLButton>("map_button", TRUE, FALSE);
	...
	LLTabContainer* tabcp = getChild<LLTabContainer>("group_money_tab_container", TRUE, FALSE);
	...
	earlierp = getChild<LLButton>("earlier_details_button", TRUE, FALSE);
	laterp = getChild<LLButton>("later_details_button", TRUE, FALSE);
	textp = getChild<LLTextEditor>("group_money_details_text", TRUE, FALSE);
	panelp = getChild<LLPanel>("group_money_details_tab", TRUE, FALSE);
...
void LLPanelGroupLandMoney::processPlacesReply(LLMessageSystem* msg, void**)
{
	...
	if (!selfp)
	{
		llinfos << "Group Panel Land and Moeny for " << group_id
