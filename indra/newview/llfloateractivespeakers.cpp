...
void LLSpeaker::onAvatarNameLookup(const LLUUID& id, const LLAvatarName& avatar_name, void* user_data)
{
	...
	if (speaker_ptr)
	{
		speaker_ptr->mLegacyName = avatar_name.getLegacyName(true); // Must keep "Resident" last names
		...
		{
			speaker_ptr->mDisplayName = avatar_name.getLegacyName(); // "Resident" last names stripped when appropriate
		}
	}
...
bool LLPanelActiveSpeakers::SpeakerMuteListener::handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
{
	...
	if (mPanel->getChild<LLUICtrl>("moderator_allow_voice", TRUE, FALSE))
	{
...
BOOL LLPanelActiveSpeakers::postBuild()
{
	...
	if (getChild<LLUICtrl>("moderator_allow_voice", TRUE, FALSE))
	{
...
{
	LLUICtrl* voice_moderation_ctrl = getChild<LLUICtrl>("moderation_mode", TRUE, FALSE);
		...
		(mVoiceChannel && mVoiceChannel->isActive()))
	{
		...
		if (participants)
		{
			...
			for (participant_it = participants->begin(); participant_it != participants->end(); ++participant_it)
			{
				...
				setSpeaker(participantp->mAvatarID, participantp->mLegacyName, LLSpeaker::STATUS_VOICE_ACTIVE, (participantp->isAvatar()?LLSpeaker::SPEAKER_AGENT:LLSpeaker::SPEAKER_EXTERNAL));
			}
		}
	}
}
