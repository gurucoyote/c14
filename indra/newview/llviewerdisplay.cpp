...
void display(BOOL rebuild, F32 zoom_factor, int subfield, BOOL for_snapshot)
{
	...
	if (gTeleportDisplay)
	{
		...
		static LLCachedControl<bool> hide_tp_progress(gSavedSettings, "HideTeleportProgress");
	...
	if (!gDisconnected)
	{
		...
		const F32 max_geom_update_time = 0.05f * gFrameIntervalSeconds; // 50 ms/second update time
...
//mk
		...
		static LLCachedControl<bool> render_use_far_clip(gSavedSettings, "RenderUseFarClip");
		LLPipeline::sUseFarClip = render_use_far_clip;
		static LLCachedControl<bool> render_delay_vb_update(gSavedSettings, "RenderDelayVBUpdate");
		LLPipeline::sDelayVBUpdate = render_delay_vb_update;
...
//mk
		...
		if (!for_snapshot)
		{
			...
			LLFastTimer t0(LLFastTimer::FTM_RENDER_UI);
			render_ui();
		}
