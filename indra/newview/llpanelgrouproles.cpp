...
{
	LL_DEBUGS("GroupPanel") << "LLPanelGroupRoles::postBuild()" << LL_ENDL;

	mSubTabContainer = getChild<LLTabContainer>("roles_tab_container", TRUE, FALSE);
	...
	case 0: // "Apply Changes"
	{
		...
		if (!apply(apply_mesg))
		{
			...
			if (!apply_mesg.empty())
			{
				...
				LLNotifications::instance().add("GenericAlert", args, LLSD(),
...
{
	if (mGroupID.isNull() || !mSubTabContainer) return;
...
{
	LL_DEBUGS("GroupPanel") << "LLPanelGroupSubTab::activate()" << LL_ENDL;
	setOthersVisible(TRUE);
}

void LLPanelGroupSubTab::deactivate()
{
	LL_DEBUGS("GroupPanel") << "LLPanelGroupSubTab::deactivate()" << LL_ENDL;
...
BOOL LLPanelGroupMembersSubTab::postBuildSubTab(LLView* root)
{
	...
	mMembersList 		= parent->getChild<LLNameListCtrl>("member_list", TRUE, FALSE);
	mAssignedRolesList	= parent->getChild<LLScrollListCtrl>("member_assigned_roles", TRUE, FALSE);
	mAllowedActionsList	= parent->getChild<LLScrollListCtrl>("member_allowed_actions", TRUE, FALSE);
	...
	LLButton* button = parent->getChild<LLButton>("member_invite", TRUE, FALSE);
	if (button)
	{
...
void LLPanelGroupMembersSubTab::handleMemberSelect()
{
		...
		if (group_role_data)
		{
			...
			if (cb_enable && count > 0 && role_id == gdatap->mOwnerRole)
			{
				...
				for (member_iter = selected_members.begin();
					 member_iter != member_end; ++member_iter)
				{
	...
	if (mEjectBtn) mEjectBtn->setEnabled(can_eject_members);
}
...
void LLPanelGroupMembersSubTab::onRoleCheck(LLUICtrl* ctrl, void* user_data)
{
	...
	if (!check_box || !self || !self->mAssignedRolesList) return;
	...
	if (mChanged)
	{
		...
		if (mNumOwnerAdditions > 0)
		{
			...
			if (gdatap->getRoleData(gdatap->mOwnerRole, rd))
			{
				...
				LLNotifications::instance().add("AddGroupOwnerWarning", args, LLSD(),
...
U64 LLPanelGroupMembersSubTab::getAgentPowersBasedOnRoleChanges(const LLUUID& agent_id)
{
	...
	if (role_change_datap)
	{
		...
		{
			bool role_in_remove_list = (std::find(roles_to_be_removed.begin(),
			...
			{
				new_powers |= current_role->second->getRoleData().mRolePowers;
			}
		}
	}
	else
...
BOOL LLPanelGroupRolesSubTab::postBuildSubTab(LLView* root)
{
	...
	mAssignedMembersList = parent->getChild<LLNameListCtrl>("role_assigned_members", TRUE, FALSE);
	mAllowedActionsList	= parent->getChild<LLScrollListCtrl>("role_allowed_actions", TRUE, FALSE);

	mRoleName = parent->getChild<LLLineEditor>("role_name", TRUE, FALSE);
	mRoleTitle = parent->getChild<LLLineEditor>("role_title", TRUE, FALSE);
	mRoleDescription = parent->getChild<LLTextEditor>("role_description", TRUE, FALSE);
...
{
	LL_DEBUGS("GroupPanel") << "LLPanelGroupRolesSubTab::apply()" << LL_ENDL;
...
{
	LL_DEBUGS("GroupPanel") << "LLPanelGroupRolesSubTab::update()" << LL_ENDL;
	...
	if (check->get())
	{
			...
			(GP_ROLE_ASSIGN_MEMBER == power || GP_ROLE_CHANGE_ACTIONS == power))
		{
			...
			if (gdatap->getRoleData(role_id, rd))
			{
				...
				LLNotifications::instance().add(warning, args, LLSD(),
...
BOOL LLPanelGroupActionsSubTab::postBuildSubTab(LLView* root)
{
	...
	mActionDescription = parent->getChild<LLTextEditor>("action_description", TRUE, FALSE);

	mActionList = parent->getChild<LLScrollListCtrl>("action_list", TRUE, FALSE);
	mActionRoles = parent->getChild<LLScrollListCtrl>("action_roles", TRUE, FALSE);
	mActionMembers  = parent->getChild<LLNameListCtrl>("action_members", TRUE, FALSE);
...
{
	LL_DEBUGS("GroupPanel") << "LLPanelGroupActionsSubTab::apply()" << LL_ENDL;
	return true;
}
