...
BOOL LLPrefsIMImpl::postBuild()
{
	...
	childDisable("log_seconds_timestamp");
	childDisable("logfile_name_datestamp");

	childSetText("busy_response", getString("log_in_to_change"));
	childSetValue("busy_response_when_away", gSavedPerAccountSettings.getBOOL("BusyResponseWhenAway"));
	...
	childSetValue("log_seconds_timestamp", gSavedPerAccountSettings.getBOOL("LogTimestampSeconds"));
...
void LLPrefsIMImpl::apply()
{
	...
	{ 
		...
		gSavedPerAccountSettings.setBOOL("BusyResponseWhenAway", childGetValue("busy_response_when_away").asBoolean());
		...
		gSavedPerAccountSettings.setBOOL("LogTimestampSeconds", childGetValue("log_seconds_timestamp").asBoolean());
...
void LLPrefsIMImpl::setPersonalInfo(const std::string& visibility, bool im_via_email, const std::string& email)
{
	...
	childSetValue("online_visibility", mHideOnlineStatus); 	 
	...
	childEnable("log_seconds_timestamp");
...
{
	LLPrefsIMImpl* self = static_cast<LLPrefsIMImpl*>(user_data);
	if (self)
	{
		LLSD args;
