...
void LLSelectMgr::requestObjectPropertiesFamily(LLViewerObject* object)
{
	...
	if (LLSelectMgr::getInstance()->mDebugSelectMgr)
	{
		llinfos << "LLSelectMgr::requestObjectPropertiesFamily(): Registered an ObjectPropertiesFamily request for object: "
				<< object->mID << llendl;
	}
...
void LLSelectMgr::processObjectPropertiesFamily(LLMessageSystem* msg, void** user_data)
{
	...
	if (LLSelectMgr::getInstance()->mDebugSelectMgr)
	{
		llinfos << "LLSelectMgr::processObjectPropertiesFamily(): Got ObjectPropertiesFamily reply for object: "
				<< id << llendl;
	}
	...
	else if (request_flags & OBJECT_PAY_REQUEST)
	{
		...
		if (ml)
		{
			ml->autoRemove(owner_id, LLMuteList::AR_MONEY);
		}
	}
...
void LLSelectMgr::renderSilhouettes(BOOL for_hud)
{
	...
	if (for_hud && avatar)
	{
		...
		LLViewerCamera* camera = LLViewerCamera::getInstance();
		glOrtho(-0.5f * camera->getAspect(), 0.5f * camera->getAspect(), -0.5f, 0.5f, 0.f, depth);
